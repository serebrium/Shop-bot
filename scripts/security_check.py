...
def check_security_issues() -> Dict[str, List[str]]:
    ...
    if 'jinja2' in packages:
        ...
        if version < '3.1.6':
            issues['critical'].append(f"Jinja2 {version} - уязвим к CVE-2024-56326, CVE-2024-56201, CVE-2025-27516, CVE-2024-34064")
    
    # Проверяем python-dotenv
    if 'python-dotenv' in packages:
        version = packages['python-dotenv']
        if version < '1.1.1':
            issues['high'].append(f"python-dotenv {version} - рекомендуется обновить до 1.1.1+")
    
    # Проверяем aiohttp
    if 'aiohttp' in packages:
        version = packages['aiohttp']
        if version < '3.12.0':
            issues['medium'].append(f"aiohttp {version} - рекомендуется обновить до 3.12.0+")
