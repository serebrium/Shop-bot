...
async def user_mode(message: types.Message):
    try:
        ...
        if cid in config.ADMINS:
            ...
            await message.answer(
                "Вы уже в пользовательском режиме.", reply_markup=ReplyKeyboardRemove()
            )
    except Exception as e:
...
async def admin_mode(message: types.Message):
    try:
        ...
        if cid not in config.ADMINS:
            ...
            await message.answer(
                "Включен админский режим.", reply_markup=ReplyKeyboardRemove()
            )
        else:
            await message.answer(
                "Вы уже в админском режиме.", reply_markup=ReplyKeyboardRemove()
            )
    except Exception as e:
